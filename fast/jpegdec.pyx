...
cdef extern from "c_jpegdev.h":
    ...
    void naive_idct_2d(int *src)
    void idct_2d(int *src)
    void ycbcr2rgb(long *ycbcr, long *rgb)
...
cdef extern from "c_huffman.h":
    # struct t_huff_table_ctx:
        # int csizes[16]
        # int last[16]
    ...
    struct t_huff_table_ctx:
        ...
        int last[16]
        unsigned char symbols[256]
        unsigned char symbols2[0x10000]
        unsigned char bits[256]
    struct t_huff_stream_ctx:
        int idx
        int sidx
        unsigned char *hdata
    
    void huff_stream_init(t_huff_stream_ctx *h, unsigned char *stream, unsigned int idx)
    void huff_table_init(t_huff_table_ctx *h, unsigned char *sizes, unsigned char *symbols)
    void huff_sync(t_huff_stream_ctx *h)
    unsigned int huff_get_symbol(t_huff_stream_ctx *h, t_huff_table_ctx *t)
...
cdef class Jpeg_decoder:
    ...
    cdef _sos_table
    cdef int _sos_table0[256]
    cdef int _sos_table1[256]

    cdef _use_ri
    cdef _ri

    cdef _bmpdata
    cdef unsigned char *_udata
    cdef _huf

    cdef _inverse_dir
    
    cdef t_huff_stream_ctx _huf_stream
    cdef t_huff_table_ctx *_huf_tables[256]
    cdef int _huf_table_count
    ...
    cdef _decode_block(self, int table_idx, int *m):
        ...
        cdef int i, is_ac, midx, tmp, hbits, rlz
        cdef int cht[2]
        cdef int *cizz_table
    ...
    cdef _decode_image_process(self, q, unitspp, int fhmax, int fvmax, int mcu_width, \
        ...
        cdef int cq[256][64] # corregir
        cdef int *cdest[256]
        cdef int **ctrans
        cdef int mmin[64]
        cdef double mmout[64]
        cdef int i,j,k,l
        cdef int x,y
        cdef int *tad, *tbd
        cdef int unit[64]
        ...
        cdef int *mvva, *mvvb, *mvvx
        cdef int **cidxs, *lcidxs
            ...
            for j in range(self._planes_count):
                ...
                # for k in range(plane['fv']):
                    ...
                    # for l in range(plane['fh']):
                        # tb = (l << 3) + tc
                        # #t1=time.clock()
                        # self._block_blt(mcu, ctrans[ic], ta, 0, tb)
                        # #tkk+=time.clock()-t1
                        # ic += 1
        ...
        #print "tiempo quant: %f\n" % tq
        #print "tiempo idct: %f\n" % ti
        #print "tiempo mcu_blt: %f\n" % tm
        #print "tiempo block_blt: %f\n" % tk
    ...
    def _m_dht(self, code, data, offset, size):
        ...
        while o < (offset + size):
            ...
            if PyBytes_Check(data):
                ...
                #PyString_AsStringAndSize(data, &csizes, &length)
